...
export default function SidebarItem({
  ...
  const baseHref =
    label.toLowerCase() === 'home' ? '/' : `/${label.toLowerCase()}`; // 'href' for sidebar menu links
    ...
    <li>
      <a
        ...
        className={`${cn(isActive && !disabled && 'menu-active')} flex items-center justify-between ${
          disabled
            ? 'pointer-events-none select-none outline-none focus:outline-none text-base-content/60'
            : ''
        }`}
      >
